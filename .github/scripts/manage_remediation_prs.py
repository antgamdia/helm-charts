...
def manage_prs(image_updates_file='image_updates.json', charts_dir='charts', dry_run=False):
    ...
    for image_ref, update_info in image_updates.items():
        ...
        if github_repo:
            ...
            if alerts:
                pr_body += f'## 🔒 Security Alerts Fixed ({len(alerts)} total)\n'
                pr_body += f'This PR addresses the following Code Scanning alerts:\n\n'
                for alert in alerts:
                    # Link CVE to NIST, link Alert number to GitHub alert
                    cve_link = f'[{alert["rule_id"]}](https://nvd.nist.gov/vuln/detail/{alert["rule_id"]})'
                    alert_link = f'[Alert #{alert["number"]}]({alert["url"]})'
                    pr_body += f'- {cve_link} - {alert_link}\n'
                pr_body += '\n'
            else:
